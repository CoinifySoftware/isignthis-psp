...
describe('_post', () => {
  describe('success', () => {
    ...
    it('should call request.post correctly', (done) => {
      const requestObject = {request: true};
      ...
      iSignThis._post(path, requestObject, (err, obj) => {
        if (err) {
          return done(err);
        }
        ...
        const callOptions = request.post.firstCall.args[0];
  ...
  describe('request error', () => {
    ...
    it('should pass an error object to the callback', (done) => {
      const iSignThis = new ISignThis({
        acquirerId,
        merchantId,
        apiClient,
