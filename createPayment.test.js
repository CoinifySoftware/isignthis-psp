...
describe('createPayment', () => {
  ...
  beforeEach(() => {
    ...
    requestStub = sinon.stub(requestPromise, 'post')
      .resolves({httpResponse: {statusCode: 201}, body: successResponse});
  ...
  describe('minimal success request/response', () => {
    it.only('correctly sends request and parses response', async () => {
      ...
      console.dir(requestStub.firstCall.args, {depth: null});
      expect(requestStub.firstCall.args[0]).to.deep.equal({
        body: {
          ...
          merchant: {
            ...
            return_url: 'https://coinify.com/payment-return'},
          transaction: {
            id: '',
            datetime: '2017-11-20T10:25:02.628Z',
            amount: '50.00',
            currency: 'DKK',
            reference: ' '},
          client: {ip: '127.0.0.1', name: undefined, address: undefined},
          account: {
          ...
          downstream_auth_type: 'bearer',
          downstream_auth_value: 'callback_auth_token',
          requested_workflow: 'SCA'
        },
        json: true,
      ...
      expect(payment.id).to.equal(successResponse.id);
    });
  });
